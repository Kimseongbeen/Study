...
const char* thingId = ""; // EC2 : 퍼블릭 IPv4 주소
const char* host = ""; //AWS 사물 상호작용 주소
...
int iTime=500;  // 로봇 동작속도, *팁:이렇게 변수식으로 만들어줘야 나중에 delay변경을 할 때 해당 변수의 값만 변경해주면 되므로 효율적입니다.
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  mode = root["mode"];
  Duty = root["duty"];
  if(mode==0) //InitSet함수로 모터를 90도로 세팅
    InitSet();
  else if(mode==1)
  ...
  else if(mode==5) { //속도 세팅
    iTime = root["valocity"];
    mode=premode;
  }
  else if(mode==9) {  //node-red에서 들어온 값을 받는 함수
      volume = root["volume"]; 
