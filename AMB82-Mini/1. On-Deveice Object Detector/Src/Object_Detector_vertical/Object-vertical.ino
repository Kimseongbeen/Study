...
#define TFT_WIDTH 240  // 세로 모드로 변경
#define TFT_HEIGHT 320 // 세로 모드로 변경
#define TFT_FPS 10
...
void setup() {
  ...
  tft.begin();
  tft.setRotation(0);  // 세로 모드로 설정 (0 또는 2)
  intoText();
    ...
    for (uint32_t i = 0; i < ObjDet.getResultCount(); i++) {
      ...
      if (itemList[obj_type].filter) {  // 항목을 무시해야 하는지 확인
        ...
        for (int i = 0; i < textLength; i++) {
          tft.drawChar(xmin + (i * 6), ymin - 10, text_str[i], ILI9341_GREEN, ILI9341_BLACK, 1);
        }
      }
    }
  }
